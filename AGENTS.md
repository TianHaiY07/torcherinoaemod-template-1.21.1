...
- **同网络加速器独占（先放置者优先）**：一个 AE 网络中只允许一台加速器工作（加速效果不叠加）——先放置的加速器正常工作；后放置的加速器停止工作（不脉冲/不耗电/不标记 working），GUI 隐藏设备列表并显示「该网络已存在AE加速器！」（`gui.torcherino_ae_mod.ae_accelerator.duplicate`），拆除其一即恢复。判定基准是持久化的「放置时刻」（`created_at_tick` NBT，首次服务端 tick 记录世界游戏时间），旧档/未知时刻按方块坐标字典序作确定性平局裁决。AE 加速火把不适用此规则（火把不是网络方块）。
...
- 单元测试：JUnit 5，现有 **15 个纯逻辑测试类**（见 §9）。**注意**：build.gradle 末段把 main 的 Minecraft/NeoForge 类路径叠加给了 test sourceSet——引用 `net.minecraft.*` 值类型的测试（BlockPos/ResourceLocation/NbtOps）依赖这份类路径，删不得。
...
- `blockentity/AEAcceleratorBlockEntity.java`（继承 AE2 `AENetworkedPoweredBlockEntity`，实现 `IUpgradeableObject`/`CommonTickingBlockEntity`/`IAccelerationSource`）：
  - **同网络加速器独占（先放置者优先）**：`createdAtTick`（NBT 键 `created_at_tick`，首次服务端 tick 记世界游戏时间，随档持久化）为先后判定基准；`networkHasOtherAccelerator` 标记「网络中是否存在不晚于本机放置的其它加速器」，由 `refreshNetworkHasOtherAccelerator`（onMainNodeStateChanged 事件即时 + rebuildTargets 周期兜底）维护，`isNotLaterThan` 为纯静态裁决（均已知比较时刻；任一未知按 `BlockPos`（Vec3i）坐标字典序平局，保证旧档/瞬态也只停一台）。标记为 true 时 `commonTick` 提前返回（不脉冲/不耗电/不 working）。静态 `isAcceleratorOwner` 供菜单/目标重建/配置卡注入统一过滤「其它加速器不是可加速设备」。火把不适用（非网络方块）。
  ...
  - 智能加速：`rebuildTargets()` 单次遍历网格产出「已登记设备」与「智能联动目标」（记入 `craftingLinkedIds`，范围由配置 `crafting.smartAccelerateScope` 控制：`ALL_ACCELERATABLE` 默认联动网内全部可加速设备—零配置兼容任意第三方 AE 工作机器；`CRAFTING_MACHINES` 仅联动 ICraftingProvider 服务宿主与 `CraftingSupport.isCraftingMachineType` 命中者）。目标按标识经去重表 `byId` 合并（登记 / 智能联动任一命中即收录一次）。**样板供应器下游联动（供应链同步加速）**：凡「被加速（登记或智能联动）的样板供应器」经 `collectDownstreamTargets` 解析其投放方向（`PatternProviderSupport.pushDirections`，方块版读 `push_direction` 状态/ALL 六向、部件版为附着面）上的相邻可加速设备，记入「下游→母源」映射 `downstreamSources`（Map<DeviceId, Set<DeviceId>>，重建期间重置，不持久化）并向 `byId` 补录未收录的下游设备。下游判定分两条路：①**网格设备**——`DeviceScanner.findAcceleratableNode` + 同网格校验 `AeGrid.gridOf(node)==grid`（含分子装配室、接口、第三方接网制造机；跨网不收录）；②**无节点原版 tick 设备（兜底）**——取不到网格节点时，相邻方块若本身有原版 tick 且不在黑名单（`DeviceScanner.vanillaTicker`/`isAcceleratableMachine`），即**完全未接 AE 网络**的第三方科技 mod 机器（如 Mekanism 设备，供应器经原版物品容器能力直接投料），仍补录为**无节点目标**（`AccelerationTarget` 的 node 为 null，由引擎按纯原版 tick 路径加速：不经网格、无激活/断电/睡眠概念，以方块是否移除作失效校验，见 §4.3 `AccelerationEngine`）。`multiplierFor` **登记优先；未登记时在「下游联动（继承母源）倍率」与「自身智能候选倍率」间取最大，再统一 `min(设定倍率, runCap())`**（`runCap()` = `rateGovernor.cap(getAccelMultiplier())`，见 §4.3 `RateGovernor`）：已登记（PLAYER/CONFIG_CARD）返回 `min(登记值, runCap)`（显式设置优先、不叠加不被覆盖）；未登记时查 `downstreamSources` 按母源 `sourceMultiplierFor`（登记值/智能倍率，不递归下游）合成 `linkedMultiplier`（多母源取最大），同时若在 `craftingLinkedIds` 且 `crafting.smartAccelerateEnabled` 开启则取当前智能倍率（以游戏时间为键每 tick 缓存：扫描被选中 `isAccelerated` 且 `isBusy()` 的 CPU 的登记倍率最大值），二者取 max 再 `min(runCap)`——供应链语义「投料方在加速、接收方不掉队」（母源倍率更高时下游同等跟进），智能语义「CPU 合成正用这台机器」（智能倍率更高时不被供应链低倍率拖累），两条独立生效取强者；两者皆 ≤1 返回 1。母源停机（登记移除/智能关闭）则下游联动自然停止。`commonTick()` 在 `AccelerationEngine.pulse` 后 `rateGovernor.sample(result.spentMs())` 把本源实际加速耗时喂入调控（无论是否 didWork 都采样，全部空闲时耗时≈0 自动回升）。**GUI 显示的仍是设定倍率（`getDeviceMultiplier`），实际倍率仅在脉冲执行时经 `multiplierFor` 生效，不写回登记表**。
...
- `blockentity/AETorcherinoBlockEntity.java`（普通 BlockEntity，**不实现 `IAccelerationSource`、不经过 `AccelerationEngine`**；服务端 tick 由方块 `getTicker` 提供，客户端不执行；目标扫描与三条加速路径已拆至同包 `TorchTargetScanner`/`TorchAccelerator`）：
  - 独立范围扫描（原始 Torcherino 式）：X/Z/Y 范围默认 3/3/2、`speed` 默认取 `ConfigDefaults.TORCHERINO_MAX_SPEED`；setter 一律 `clampRange(v, RuntimeConfig 上限)`，变化后触发 `forceRescan` + 重建 `TorcherinoWakeup` 覆盖 + 存档 + `sendBlockUpdated`。NBT 键 `x_range/z_range/y_range/speed/enabled`，加载后 clamp 并保底 speed≥1、旧档无开关标签默认开启。`isActive() = enabled && speed > 1 && !isRangeEmpty()`。**模型开关同步**：`setEnabled` 变化即时经 `AETorcherinoBlock.applyEnabledState` 写入方块状态（`level.setBlock(..., Block.UPDATE_CLIENTS)`）驱动 off/点亮模型切换；每次加载首帧 `syncEnabledBlockStateOnce` 兜底一次，修复旧档状态缺 `enabled` 属性导致的变体缺失（正常状态零开销空操作）。
  - 目标扫描（**分片增量 + 自适应退避**，参照 RTS 输电塔的供电范围扫描）：一整圈影响范围扫描分摊到 `torcherino.scanIntervalTicks`（默认 20）tick 内完成（`maybeScan`/`scanRange` 按展平下标分片，单格仅 1 次 `getBlockEntity` + 跳过未加载区块 `level.isLoaded`），其余 tick 完全跳过（零扫描成本）；一圈结束 `commitTargets` 比较目标集是否变化（`sameTargets` 按位置集合比较），有变化 → 周期收窄回密集发现周期，无变化 → 周期翻倍退避至 `torcherino.scanBackoffMaxTicks`（默认 200，稳态扫描成本降到 ≤1/10）。**窗口自适应**：`windowFor(base, size, maxCellsPerTick)` 在范围极大（服务端调高 `maxXzRange`/`maxYRange`）时把分片窗口线性拉长，把单 tick 实际查询格数钳制在 `torcherino.scanMaxCellsPerTick`（默认 512）内，防止超大范围单 tick 全量遍历造成主线程尖峰；小范围保持基础窗口不变。`forceRescan()` 立即收窄回密集周期并置扫描门为当前时刻（下个 tick 即开扫一圈），由 `TorcherinoWakeup`（范围内方块放置/破坏事件）与 `onConfigChanged` 调用——退避稳态省成本、事件驱动保证新设备及时识别，两者兼得。目标按 `pendingTargets` 累积、圈末一次性提交 `targets`（提交期间旧列表持续生效，无加速空窗）。执行前校验方块实体类型未变，防止误加速被替换方块。
  - `isInRange(pos)` 供 `TorcherinoWakeup` 做事件 AABB 精确校验；`onLoad`/`setRemoved` 负责注册/注销空间哈希。
  - 三条加速路径委托 `TorchAccelerator.accelerate`（同一目标各自并行生效）：AE 网格 tick（`IGridTickable` 重复 `tickingRequest(node,1)`，返回 `SLEEP` 时 `ITickManager.sleepDevice` 早退）、方块实体 ticker（`EntityBlock.getTicker` 重复调用）、随机 tick（`BlockState.randomTick`，**概率式分摊**：把「加速 N 次」折成「提高每 tick 触发概率」，单格单 tick 至多触发 1 次 `randomTick`，使随机 tick 加速工作量与 speed 基本解耦，参照原版 Torcherino，见 `TorchAccelerator.randomTickDenominator/randomTickHit` —— 不是每 tick 对每格暴力 `speed-1` 次调用）。
  - **性能预算**：每 tick 先 `budget().resetTick()`——`budget()` 经 `core/SourceBudget` 读生效值 = 配置 `budget.tickCallsPerSource` 经 `AdaptiveThrottle.INSTANCE.adjust`（实例仅在生效预算变化时重建）；三条路径每次调用前 `budget.request(1)` 按次申请，额度耗尽即停止剩余调用。火把因此与加速器共享同一套 TPS 自适应节流（拖垮 TPS 时逐档削峰）。**源级耗时调控**：tick 期间计时「本火把贡献的加速耗时」（仅计加速执行循环、不计范围扫描），喂入本类持有的 `torchRateGovernor`；传给 `TorchAccelerator.accelerate` 的倍率是 **`effectiveSpeed() = min(speed, torchRateGovernor.cap(speed))`**（设定倍率只作总阈值，见 §4.3 `RateGovernor`），随机 tick 分母同步用该有效倍率。
...
- `blockentity/TorcherinoWakeup.java`（**空间哈希事件唤醒**，参照 RTS 输电塔 `PowerTowerWakeup`，主类经 `NeoForge.EVENT_BUS.register(TorcherinoWakeup.class)` 注册）：监听服务器方块放置/破坏（`BlockEvent.EntityPlaceEvent`/`EntityMultiPlaceEvent`/`BreakEvent`），若事件位置落在某火把影响范围内则调用 `forceRescan` 立即密集重扫，补充自适应退避下「新增设备响应慢」的短板。用空间哈希（`CELL=32` 网格单元，`Map<ServerLevel, Map<Long, List>>`，维度经 WeakHashMap 卸载回收）而非线性遍历全部火把，复杂度 O(覆盖该单元的重叠火把数)。仅服务端生效，`onLoad`/`setRemoved`/`onConfigChanged` 注册/注销/重建覆盖。
- `blockentity/TorchTargetScanner.java`：火把影响范围目标**解析器**（与扫描调度解耦，纯逻辑可单测）。提供 `bounds(self, xRange, yRange, zRange)`（外接盒 + 展平 `size`）、`offsetForIndex(index, bounds)`（展平下标→相对火把坐标偏移，下标按 Z 最快/Y 次/X 最慢排列）、`windowFor(baseTicks, cellCount, maxCellsPerTick)`（分片窗口长度自适应：范围极大时按单 tick 单元格预算把窗口拉长）、`resolve(level, self, pos)`（单格解析为不可变 `Target` 快照或 null，判定候选为「AE 网格宿主 / 带方块实体 ticker / 随机 tick」，排除自身与其它火把防互相递归）。`Target` 判定与加速路径解耦、独立可测。
...
- `blockentity/ConfigCardCleanup.java`（服务端「设备/合成 CPU 组被破坏」的即时清理协调器，监听 `BlockEvent.BreakEvent`、爆炸事件、方块实体 `setRemoved` 与 `ServerTickEvent.Post`）：
  ...
  - **合成 CPU 组（AE2 原版 + AdvancedAE 大型 CPU 软兼容）**：成员块拆除经 `CraftingSupport.cpuGroupOf(be,true)` 快照整组成员坐标走 `CpuRemoval` 整组通道，等 `CPU_SETTLE_TICKS`(4) tick 让集群「整体解散+剩余重算再成型」结算完成后观察剩余成员真实成型状态：无残留或残留拆成多个独立组 → `ConfigCardData.removeCpuBinding` 删 CPU 条目连带裁剪几何；恰一组残留 → `replaceCpuBinding` 改写为新集群最小角标识与新几何（部分破坏仍成型的自适应，卡入槽即触发重注入）。裁决只依赖结构标识/坐标，不持有集群对象引用（集群销毁后不再可信）。**观察区含未加载区块时最多重排等待 `CPU_SETTLE_MAX_RETRIES`(50) 轮**（防止永不加载的区域无限排队），超限放弃并交由下方周期兜底。
  - 自清理：卡槽内卡片经 `inventory.setItemDirect` 改写触发 `onHostInventoryChanged → syncConfigCardDevices()` 自动精确重同步；玩家背包内卡片在裁决时同步改写。
  - **周期兜底（`reconcileInventoryCards`，每 `INVENTORY_RECONCILE_INTERVAL`(40) tick）**：逐张扫描<b>在线玩家背包</b>配置卡，复用 `ConfigCardBinding.reconcileCpuBindingsOnCard`（世界几何校验 `cpuGroupAt`+`cpuGroupsWithin`，删/换绑语义同 `resolveCpuFate`）清理「CPU 经非事件途径失效（改成 L 形等无效形状）、拆除事件漏网、同网络定位失败」时残留的卡上记录。它不依赖破坏事件与网格，是事件清理的兜底；箱子/ME 存储等不可枚举容器与离线玩家无法清扫，留待卡片下次被取用/放入加速器时经槽位自愈。此路径不处理普通设备（走 `purgeRemovedBlockDevices`）。
...
- `api/`：
  ...
  - `AccelerationTarget`（record：id + node + tickable + vanillaTicker + blockEntity，node/blockEntity 均可为 null）：两类目标——网格目标（node 非空，载体为 tickable/vanillaTicker 二选一，blockEntity 供原版 tick 路径的方块实体引用）与**无节点目标**（node 为 null = 未接 AE 网络的纯原版 tick 下游方块，vanillaTicker 必有、blockEntity 必传，只能由样板供应器下游联动产生）；`isDetached()`/`belongsTo(grid)` 内部把「节点销毁时 getGrid() 抛 ISE」转译为 null（安全护栏，勿改），无节点目标不适用这两个方法（引擎对其走专用分支）。
...
- `core/`：
  - `AccelerationEngine.pulse(source)`：AE 加速器侧的脉冲执行器（火把已独立实现，见 4.2）。流程：`!source.isActive()` 直接返回 `NONE` → `budget.resetTick()` → 逐目标依次判定：① 网格目标 `target.isDetached() || !target.belongsTo(expectedGrid)` → 计 skippedDetached、`markTargetsDirty`（下轮剔除）；**无节点目标**（node 为 null，见 §4.2 下游联动兜底）改以「blockEntity 非空且未移除、vanillaTicker 非空」作失效校验，同样计 skippedDetached / 标脏；② 网格目标节点未激活跳过（无节点目标无激活概念，跳过本步）；③ `multiplierFor(id)-1 <= 0` 跳过（倍率 1 即不加速；**先倍率后睡眠**）；④ `getTickingRequest(node).isSleeping()` 跳过（空闲设备催促无意义；仅网格 tick 目标，原版 tick 目标跳过）；⑤ 预算耗尽 → 结束脉冲；⑥ 网格 tick 目标 `alertDevice` 催促后在**同一 tick 内**循环 `tickingRequest(node, 1)`（该逐次推进循环共用 `GridTickAccelerator.tick`），返回 `TickRateModulation.SLEEP` 立即 break（设备处理中睡眠后继续推进会越过其状态机边界）；原版 tick 目标（含无节点目标）不经网格 tick 管理器，取目标上的 vanillaTicker / blockEntity（或 node.getOwner() 兜底）直接反复执行 `vanilla.tick`。**性能约束**：本方法位于每 tick 路径，禁止分配对象、字符串拼接、日志、遍历 `grid.getNodes()`。
  ...
  - `RateGovernor`（**源级加速耗时调控，主调控**，每源一个实例——加速器/火把各持一个，互不干扰）：按「本源自身贡献的加速耗时」把<b>实际加速倍率</b>动态下调，<b>设定倍率只作总阈值、永不超过去</b>。状态机为缩放因子 `factor ∈ (0,1]`（1=用足设定倍率）：本 tick 加速耗时 EMA ≥ `rate.sourceMsLimit × rate.tightenRatio`（默认 15.0ms × 1.0，走 `RuntimeConfig.rate*()`）时因子<b>即时</b>压到 `min(factor, sourceMsLimit / emaMs)`；耗时 EMA < `sourceMsLimit × relaxRatio`（默认 0.7）时因子<b>每 tick 回升</b> `+RELAX_STEP(0.1)`；中间滞回带保持。实际倍率 = `clamp(round(baseCap × factor), 1, baseCap)`（`cap()`：无样本返回 baseCap 不干预）。**与 4.1 的整 tick `AdaptiveThrottle` 次数预算作用层级不同**：后者保留作「极端高倍率 / 超大范围」下的兜底，两者互不冲突。纯函数 `nextEma/nextFactor/effectiveCap` 供单测。
...
- `config/`：
  ...
  - `ModConfig`：Server + Client 两段 Spec。Server 分组与键：`accelerator.baseMultiplier/cardMultipliers/maxMultiplierCap`（-1 不限）、`budget.tickCallsPerSource`（-1 不限）、`adaptive.enabled/floorCallsPerSource/tightenMs/relaxMs`（TPS 自适应节流，默认 true/256/45/35，收紧起点即 floor，超时自动逐档减半）、`rate.sourceMsLimit/emaAlpha/tightenRatio/relaxRatio`（源级加速耗时调控，默认 15.0/0.25/1.0/0.7，见 §4.3 `RateGovernor`）、`power.perTick/perUpgradeCard/perAcceleratedDevice/bufferFraction`、`cache.rebuildIntervalTicks`、`menu.deviceListRefreshTicks`、`grid.acceleratableBlacklist`、`grid.craftingMachineExtraTypes`、`crafting.smartAccelerateEnabled/smartAccelerateScope`、`debug.enabled/sampleIntervalTicks`、`torcherino.maxSpeed/maxXzRange/maxYRange`（默认 4/8/4）、`torcherino.scanIntervalTicks/scanBackoffMaxTicks/scanMaxCellsPerTick`（分片扫描窗口、退避上限与单 tick 单元格预算，默认 20/200/512）、`torcherino.randomTickRate`（随机 tick 概率式加速的倍率系数，默认 4）。Client 段：`client.cacheFilteredList`、`client.renderBracketHighlight`。
...
- `network/PatternProviderSupport`：**样板供应器下游联动辅助**（纯逻辑可单测，不触碰 AE2 内部对象）：`isPatternProvider(owner)`（`instanceof PatternProviderLogicHost`，方块版 `PatternProviderBlockEntity` / 部件版 `PatternProviderPart` 统一命中，第三方供料设备亦兼容）、`pushDirections(owner)`（委托 `getTargets()`：方块版 = `push_direction` 状态单方向或 ALL 六向；部件版 = 附着面）、`downstreamPositions(origin, dirs)`（投放起点沿方向集偏移一格，无重复）、`linkedMultiplier(sources, multiplierOf)`（下游应继承的联动倍率 = 各母源生效倍率最大值，≤1 母源忽略）。消费方为 `AEAcceleratorBlockEntity` 的 `collectDownstreamTargets`；网格桥接（相邻方块节点解析、同网格校验）不在本类，由调用方经 `DeviceScanner`/`AeGrid` 完成。
...
- **源级耗时调控（`RateGovernor`）语义勿破坏**：设定倍率只是总阈值，实际倍率只**往下调、永不往上超**；每源独立计量（加速器/火把各自持有实例），「先倍率后睡眠」的判定顺序、以及「GUI 显示设定倍率、实际倍率仅在脉冲执行时生效」这层解耦都是刻意设计。`rateGovernor.sample` 无论 `didWork` 都调用（全部空闲时耗时≈0，实际倍率自动回升），勿改成仅在 `didWork` 时采样。
...
- **同网络加速器独占语义（勿破坏）**：加速效果不叠加；后放置者停止工作并提示（`gui.<modid>.ae_accelerator.duplicate`）。判定键为持久化 `created_at_tick`，事件即时 + 周期兜底双维护；`isAcceleratorOwner` 过滤不可少（菜单列表、`isDeviceInGrid` 校验、`rebuildTargets`、`ConfigCardBinding` 注入均须排除其它加速器）。
- **样板供应器下游联动语义（勿破坏）**：加速器对「被加速（登记或智能联动）的样板供应器」生效时，其投放方向上的可加速相邻设备继承母源生效倍率——包括<b>未接 AE 网络的纯原版 tick 机器</b>（无节点目标，如 Mekanism 设备：取不到网格节点时以「原版 tick 且不在黑名单」兜底收录，由引擎按纯原版 tick 路径加速，玩家无法在 GUI 中直接勾选它们）。玩家对下游设备显式设置倍数优先（不叠加）；下游设备的「取消加速」只移除玩家来源，若母源仍被加速则联动继续（要停止整条供应链应停母源）；母源倍率查询不递归下游（防循环放大）。映射仅内存态（重建期间重置），不持久化、不进 GUI 设备列表（GUI 显示设定倍率，联动倍率只在脉冲执行时生效）。
...
- **P6 样板供应器下游联动（功能新增）**：「加速器对样板供应器生效 → 投放方向下游接收设备一并加速」——新增 `network/PatternProviderSupport`（宿主识别/投放方向/下游坐标/联动倍率合成的纯逻辑，可单测）；`AEAcceleratorBlockEntity` 增加 `downstreamSources`（下游→母源映射，重建期间重置不持久化）与 `collectDownstreamTargets`（下游判定两条路：① 网格设备——相邻可加速且同网格补录目标；② 无节点原版 tick 设备兜底——未接 AE 网络的第三方机器按纯原版 tick 收录），`AccelerationTarget` 扩展为五组件（node 可空 + blockEntity），`AccelerationEngine` 增加无节点目标专用判定分支（以方块移除作失效校验）；`multiplierFor` 登记优先、未登记时下游联动与自身智能候选取最大（多母源取最大、不递归防循环、玩家显式设置优先）。覆盖普通加速与智能加速两条路径；火把为范围扫描型加速源，不适用该联动（不在范围）。

## 9. 单元测试清单（`src/test/java`，共 16 个，`gradlew test` 全绿）
...
| `RateGovernorTest` | core | EMA 收敛、因子推进（超限即时下压/滞回保持/逐 tick 回升封顶）、实际倍率上下界钳制、整机 sample→cap 流程 |
| `GridTickAcceleratorTest` | core | 黑盒推进循环（全额调用/SLEEP 早退/零调用/空参防御/不解析节点字段） |
| `TorchTargetScannerTest` | blockentity | 范围外接盒体积/边界、展平下标↔相对偏移的角与中心、整盒格子双射、零范围仅含自身、分片窗口长度自适应 |
| `TorchAcceleratorTest` | blockentity | 概率式随机 tick 加速的分母钳制/倍率单调递减、命中判定区间、随机 tick 关闭时不命中 |
| `AcceleratorOrderTest` | blockentity | 同网络独占的先后裁决（放置时刻比较/任一未知与两者未知的坐标平局裁决） |
...
| `PatternProviderSupportTest` | network | 投放方向→下游坐标展开（单方向/六向无重复）、联动倍率合成（空集/多母源取最大/无效忽略） |
