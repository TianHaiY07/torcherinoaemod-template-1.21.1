 ...
 * <p>
 * 取代旧实现用字符串承载身份的做法（纯坐标 long、"坐标|朝向"、"cpu:坐标"），
 * 把「维度 / 坐标 / 朝向 / 种类」四个语义字段显式建模：
 * <ul>
 *   <li><b>维度</b>：修复旧实现用 {@code BlockPos.asLong()} 导致跨维度同坐标互相串扰的缺陷
 *       （加速器与配置卡的绑定都因此会把另一个维度的同坐标方块认成同一个目标）；</li>
 *   <li><b>朝向</b>：区分挂在同一线缆坐标上、不同朝向的多个部件；方块实体与 CPU 为 {@code null}；</li>
 *   <li><b>种类</b>：区分方块实体 / 部件 / 合成 CPU，不再依赖字符串前缀约定。</li>
 * </ul>
 * 提供三种编码形式，各处按需取用：
 * <ul>
