...
/**
 * 加速器能耗模型：计算加速器每 tick 需要从 AE 网络抽取的能量。
 * <p>
 * 公式：{@code 基础值 + 升级卡数 × 每张卡增量 + 被加速设备数 × 每台增量}，
 * 默认为 {@code 1.0 + 卡数 × 0.5 + 设备数 × 0.5}（单位 AE/t）。
 * <p>
 * 注意：能耗与实际加速倍率是<b>脱钩</b>的（极高倍率与 2 倍加速耗电相同），
 * 这是有意保留的数值设计；若要改为随倍率增长，统一修改本类即可，
 * 而不是散落到调用方。
 * <p>
 * 本类不依赖 Minecraft 运行时，可直接单测。运行时系数由调用方经
 * {@code RuntimeConfig} 读取并传入带参 {@link #requiredPerTick(double, double, double, int, int)}；
 * 以下 {@code DEFAULT_*} 常量引用 {@link ConfigDefaults}，供默认便捷重载与单测基准。
 */
